...
class TagHistoryConnector(DatabaseConnection):
    ...
    def __init__(self, tag_set=None, *args, **kwargs):
        ...
        if tag_set is not None:
            tag_set_dict = {
                'lam1': {'lot_number': 'mahlo/lam1/batchid'},
                'lam2': {'lot_number': 'mahlo/lam2/batchid'},
                }
            tag_set_dict['lam0'] = tag_set_dict['lam1']
            for field, ilike_str in tag_set_dict[tag_set].items():
                lg.debug('setting %s field using %s', field, ilike_str)
                self.tag_ids[field] = self.tag_id_query(ilike_str)
    ...
    def get_recent_lots(self, lot_count=5):
        ...
        lot_query = sql.SQL(r'''SELECT stringvalue FROM sqlth_1_data WHERE tagid = %s ORDER BY t_stamp 
        DESC LIMIT %s;''')
        self.crs.execute(lot_query, (self.tag_ids['lot_number'], lot_count))
