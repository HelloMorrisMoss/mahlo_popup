...
- [x] T056 [US11] Implement touch scrolling in `NavFrame` with vertical axis locking (HLP-053, HLP-055)
- [x] T057 [US11] Implement touch scrolling in `ArticleViewer` with axis stability and `bindtags` (HLP-053, HLP-054,
  HLP-055)
- [x] T058 [US11] Implement tap vs. swipe threshold logic to distinguish between scrolling and selection/link clicks (
  HLP-054)
...
- [x] T063 [US11] Implement kinetic/inertial scrolling logic in `TouchScroller` with exponential decay (HLP-057)
- [x] T064 [US11] Implement drift suppression for slow touch releases (HLP-058)
- [x] T065 [US11] Implement immediate interruption of inertial scroll on `ButtonPress` (HLP-059)
- [x] T066 [US11] Implement boundary enforcement to prevent scrolling beyond content limits (HLP-060)
- [x] T067 [US11] Implement boundary feedback (hard stop or resistance) (HLP-061)
- [x] T068 [US11] Verify kinetic scrolling and boundary behavior on touch device (SC-009)
- [x] T069 [US11] Polish touch scrolling: fix 'jump back' by resetting anchor at release and smooth velocity (HLP-057,
  HLP-058)
